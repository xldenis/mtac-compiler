...
Fixpoint app {A} (f : A -> tactic) (ls : mlist A) : tactic :=
  match ls with
...
Fixpoint all {A} (f : A -> tactic) (ls : mlist A) : tactic :=
  match ls with
  | [m:] => fun _ => M.failwith "empty list"
  | x :m: ls' => f x ;; all f ls'
  end.
  ...
  mrun (match_goal with
    ...
    | [[? (A : Type) (B : Type) (F : A -> B) (X : A) (Y : A) (G : Prop) | (H : F X = F Y) |- G]] =>
      ...
      || (minjection H &> match_goal with
        ...
        end) (* This fails when dealing with Equality.v *)
  end) || match goal with
    | [ H : ?F ?X ?U = ?F ?Y ?V |- ?G ] =>
...
Ltac assertCons x := assert x by constructor.
Definition assertCons {A} (x: A) : tactic :=
  T.ltac ("assertCons") [m:Dyn x].
...
Ltac inster e trace :=
  ...
  match type of e with
    ...
    | _ =>
      ...
      match trace with
        | (_, _) => let trace_list := tuple_to_list trace in
                ...
                mmatch (M.type_of T) with
                  | Prop =>
                    (** [e] should be thought of as a proof, so let's add it to the context, and also add a new marker hypothesis recording our choice of trace. *)
                     (generalize e &> intro H &> assertCons (done (m: trace, tt)))
                  | _ =>
                    (** [e] is something beside a proof.  Better make sure no element of our current trace was generated by a previous call to [inster], or we might get stuck in an infinite loop!  (We store previous [inster] terms in second positions of tuples used as arguments to [done] in hypotheses.  Proofs instantiated by [inster] merely use [tt] in such positions.) *)
                     ((all ltac:(fun X => (* i don't believe this is ver called.... *)
                      match goal with
                        | [ H : done (_, X) |- _ ] => fail 1
                        | _ => idtac
                      end) trace_list) &>
